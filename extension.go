...
var kinds = map[string]rule.KindInfo{
	"oci_image": {
		NonEmptyAttrs:  map[string]bool{"tars": true},
...
func (e *Extension) GenerateRules(args language.GenerateArgs) language.GenerateResult {
	...
	i := slices.IndexFunc(args.File.Rules, func(r *rule.Rule) bool {
		return r != nil && r.Kind() == "java_binary"
	})
	...
	tar := rule.NewRule("tar", "layer")
	jar := args.File.Rules[i].Name() + "_deploy.jar"
	tar.SetAttr("srcs", []string{jar})

	/* 	img := rule.NewRule("oci_image", "all_files") */
	return language.GenerateResult{
		Gen:     []*rule.Rule{tar},
